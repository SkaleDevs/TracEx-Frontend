...
const SellProduct = () => {
    ...
    const sendNotification = async() => {
      try {
        const apiResponse = await PushAPI.payloads.sendNotification({
          ...
          payload: {
            title: `Gds have beeen diispatched`,
            body: `to : ${address}`,
            cta: 'jk',
  ...
  const submitHandler = async () => {
    ...
    console.log(address, barcodeID);
    // const userDetails = { name, email, address };
    let party = await SupplyChainContract.sellProduct(address, barcodeID,new Date().getTime(), {
      gasLimit: 5000000
    });
    sendNotification();
  ...
  return (
    ...
    >
      <CardHeader title="Transfer goods" />
      <CardContent>
